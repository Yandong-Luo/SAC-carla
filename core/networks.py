...
def control_branch(inputs: dict, units: int, num_layers: int, activation='swish6'):
    x = inputs['dynamics']
...
class PolicyNetwork(tf.keras.Model):
    ...
    def __init__(self, agent, inputs: Dict[str, Input], name='PolicyNetwork', **kwargs):
        ...
        print("num_actions",agent.num_actions)
        self.agent = agent
    ...
    def policy_branch(self, inputs: Dict[str, Input], num_actions, **kwargs):
        ...
        # auxiliary outputs
        similarity = Dense(units=1, activation='tanh', bias_initializer='glorot_uniform',
                           name=f'pi-similarity')(branch)

        speed = Dense(units=1, activation='double_sigmoid', bias_initializer='glorot_uniform',
...
class CARLANetwork(Network):
    ...
    def __init__(self, agent, control_policy: dict, control_value: dict, dynamics: dict, update_dynamics=False):
        ...
        get_custom_objects().update({'swish6': utils.swish6})
        get_custom_objects().update({'double_sigmoid': utils.double_sigmoid})
        get_custom_objects().update({'exp_sigmoid': utils.exp_sigmoid})
        ...
        self.target_value = models.clone_model(self.value)
    ...
    def value_head(self, layer: Layer, index=0, exponent_scale=6.0, **kwargs):
        ...
        base = Dense(units=1, activation='tanh', bias_initializer='glorot_uniform',
                     name=f'v-base-{index}')(layer)
        exp = Dense(units=1, activation='exp_sigmoid', bias_initializer='glorot_uniform',
...
class CARLANetworkBasedSAC(Network):
    """The CARLAgent network"""
    ...
    def __init__(self, agent, control_policy: dict, control_value: dict, dynamics: dict, update_dynamics=False):
        ...
        self.exp_scale = 6.0
        self.value = self.value_network(**control_value)
        self.target_value_network = models.clone_model(self.value)
        self._update_target_value_network()
        self.last_value = tf.zeros((1, 2), dtype=tf.float32)  # (base, exp)

        # Q value model
        self.Q_value_network1 = self.Q_value_network(**control_value)
        self.Q_value_network2 = models.clone_model(self.Q_value_network1)
    ...
    def _update_target_value_network(self):
        self.ema.apply(self.value.trainable_variables)
        for target_value_network_para, value_network_para in zip(self.target_value_network.trainable_variables, self.value.trainable_variables):
    ...
    # =================================================================
    # Q Value Network
    def Q_value_network(self, **kwargs) -> Model:
        exp_scale = kwargs.pop('exponent_scale', 6.0)
        components = kwargs.pop('components', 1)
        q_value, speed, similarity = self.Q_value_branch(0, exp_scale, components, **kwargs)

        outputs = dict(q_value=q_value, similarity=similarity, speed=speed)
        return Model(inputs=self.intermediate_inputs, outputs=outputs, name='Q-Value-Network')
    
    def Q_control_branch(self, inputs: dict, units: int, num_layers: int, activation='swish6'):
        x = tf.concat([inputs['dynamics'], inputs['action']], axis=1) 
        for _ in range(num_layers):
            x = BatchNormalization()(x)
            x = Dense(units, activation=activation, bias_initializer='glorot_uniform')(x)
    ...
    def Q_value_branch(self, index: int, exp_scale: float, components: int, **kwargs):
        branch = self.Q_control_branch(self.intermediate_inputs, **kwargs)
        q_value = self.value_head(branch, index, exponent_scale=exp_scale, components=components)
        # auxiliary outputs
        speed = Dense(units=1, activation='double_sigmoid', bias_initializer='glorot_uniform',
                      name=f'v-speed-{index}')(branch)
        similarity = Dense(units=1, activation='tanh', bias_initializer='glorot_uniform',
        ...
        return q_value, speed, similarity
    
    def q_value_predict(self, inputs):
        return self.Q_value_network1(inputs, training=False)['q_value']
