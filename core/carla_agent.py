...
from core.networks import CARLANetwork, CARLANetworkBasedSAC
...
class CARLAgent(PPOAgent):
    ...
    def __init__(self, *args, aug_intensity=1.0, clip_norm=(1.0, 1.0, 1.0), name='carla', load_full=True, eta=0.0,
        ...
        network_spec.setdefault('network', CARLANetworkBasedSAC)
        ...
        self.network: CARLANetworkBasedSAC = self.network
    ...
    def all_batch_tensors(self) -> Union[tuple, dict]:
        states, advantages, actions, log_probabilities, returns, done, next_states = super().all_batch_tensors()
        states['action'] = self.memory.actions
        next_states['action'] = actions
        ...
        return states, advantages, log_probabilities, speed, similarity, returns, done, next_states
    ...
    def get_policy_gradients(self, batch):
        ...
        if self.should_update_dynamics:
            ...
            with tf.GradientTape(persistent=True) as tape:
                ...
                print("get policy gradient from carla agent class",dynamics_out)
            ...
            # alpha_gradients = tape.gradient(alpha_loss, [self.log_aplha])
    ...
    #     states, advantages, log_probabilities, speed, similarity = batch
    #     if self.should_update_dynamics:
    #         print("get policy gradient from carla agent class",states)
    #         with tf.GradientTape(persistent=True) as tape:
    #             # dynamics prediction
    #             dynamics_out = self.network.dynamics_predict_train(states)
    #             new_batch = (dynamics_out, advantages, log_probabilities, speed, similarity)
    ...
    #             # policy prediction
    #             # loss = self.policy_objective(batch=new_batch)
    #             # SAC policy prediction
    #             loss, alpha_loss = self.SAC_policy_objective(batch=new_batch)
    ...
    #         del tape
    #         return loss, alpha_loss, dict(policy=policy_gradients, dynamics=dynamics_gradients), alpha_gradients
    #     else:
    #         dynamics_out = self.network.dynamics_predict_train(states)
    #         new_batch = (dynamics_out, advantages, log_probabilities, speed, similarity)
    ...
    def get_all_update(self, batch):
        states, advantages, log_probabilities, true_speed, true_similarity, returns, done, next_states = batch
        # if self.should_update_dynamics:
        with tf.GradientTape(persistent=True) as tape:
            # dynamics prediction
            dynamics_out = self.network.dynamics_predict_train(states)
            # new_batch = (dynamics_out, advantages, log_probabilities, speed, similarity)
            policy = self.policy_predict(dynamics_out)
            policy_action = policy['actions']
            # print(policy_action)
            log_prob = policy['log_prob']
            # entropy = tf.reduce_mean(policy['entropy'])
            policy_speed = policy['speed']
            policy_similarity = policy['similarity']

            log_prob = tf.reduce_mean(log_prob, axis=-1, keepdims=True)
            new_Q_value = tf.math.minimum(self.network.Q_value_network1([dynamics_out, policy_action])['q_value'], self.network.Q_value_network2([dynamics_out, policy_action])['q_value'])
            policy_loss = tf.reduce_mean(tf.exp(self.log_aplha) * log_prob - new_Q_value)

            # aux losses
            policy_speed_loss = 0.5 * tf.reduce_mean(losses.MSE(y_true=true_speed, y_pred=policy_speed))
            policy_similarity_loss = 0.5 * tf.reduce_mean(losses.MSE(y_true=true_similarity, y_pred=policy_similarity))

            total_policy_loss = policy_loss + policy_speed_loss + policy_similarity_loss

        policy_gradients = tape.gradient(total_policy_loss, self.network.policy.trainable_variables)
        self.policy_optimizer.apply_gradients(zip(policy_gradients, self.network.policy.trainable_variables))
        dynamics_gradients = tape.gradient(total_policy_loss, self.network.dynamics.trainable_variables)
        self.dynamics_optimizer.apply_gradients(zip(dynamics_gradients, self.network.dynamics.trainable_variables))

        with tf.GradientTape() as tape:
            # alpha
            alpha_loss = - tf.exp(self.log_aplha) * (tf.reduce_mean(tf.exp(log_prob) * log_prob) + self.mini_entropy)
        
        alpha_gradients = tape.gradient(alpha_loss, [self.log_aplha])
        self.alpha_optimizer.apply_gradients(zip(alpha_gradients, [self.log_aplha]))

        with tf.GradientTape() as tape:
            prediction = self.value_predict(dynamics_out)
            values, value_speed, value_similarity = prediction['value'], prediction['speed'], prediction['similarity']
            ...
            # values_ = tf.stop_gradient(new_Q_value - tf.exp(self.log_aplha) * log_prob)
            # value_loss = tf.reduce_mean(losses.mean_squared_error(values_, values))

            # auxiliary losses:
            speed_loss = tf.reduce_mean(losses.MSE(y_true=true_speed, y_pred=value_speed))
            similarity_loss = tf.reduce_mean(losses.MSE(y_true=true_similarity, y_pred=value_similarity))

            total_value_loss = 0.25 * (value_loss + speed_loss + similarity_loss)
        value_network_grad = tape.gradient(total_value_loss, self.network.value.trainable_variables)
        self.value_optimizer.apply_gradients(zip(value_network_grad, self.network.value.trainable_variables))
        
        next_dynamics_out = self.network.dynamics_predict_train(next_states)
        target_value = tf.stop_gradient(self.network.target_value_network(next_dynamics_out, training=True)['value'])
        Q_ = returns + self.gamma * (1 - tf.cast(done, tf.float32)) * target_value

        with tf.GradientTape() as tape:
            Q_1 = self.network.Q_value_network1([dynamics_out, next_states['action']], training=True)['q_value']
            Q_1_loss = tf.reduce_mean(losses.mean_squared_error(Q_, Q_1))
        Q_network_1_grad = tape.gradient(Q_1_loss, self.network.Q_value_network1.trainable_variables)
        self.Q1_optimizer.apply_gradients(zip(Q_network_1_grad, self.network.Q_value_network1.trainable_variables))

        with tf.GradientTape() as tape:
            Q_2 = self.network.Q_value_network2(dynamics_out)['q_value']
            Q_2_loss = tf.reduce_mean(losses.mean_squared_error(Q_, Q_2))
        Q_network_2_grad = tape.gradient(Q_2_loss, self.network.Q_value_network2.trainable_variables)
        self.Q2_optimizer.apply_gradients(zip(Q_network_2_grad, self.network.Q_value_network2.trainable_variables))
    ...
    def apply_dynamics_gradients(self, gradients):
        # print("apply_dynamics_gradients=apply_dynamics_gradients")
    ...
    def policy_objective(self, batch):
        ...
        # print("policy_objective policy_objective policy_objective")
        log_prob = policy['log_prob']
        entropy = tf.reduce_mean(policy['entropy'])
        speed = policy['speed']
    ...
    def get_value_gradients(self, batch):
        ...
        print("get_value_gradients get_value_gradients get_value_gradients")
        if self.should_update_dynamics:
            with tf.GradientTape(persistent=True) as tape:
                # dynamics prediction
                dynamics_out = self.network.dynamics_predict_train(states)
...
class CARLAMemory(PPOMemory):
    def __init__(self, state_spec: dict, num_actions: int, time_horizon: int):
        ...
        if self.simple_state:
            ...
            self.next_states = tf.zeros(shape=(0, self.time_horizon) + state_spec.get('state'), dtype=tf.float32)
        else:
            for name, shape in state_spec.items():
                self.states[name] = tf.zeros(shape=(0, self.time_horizon) + shape, dtype=tf.float32)
                self.next_states[name] = tf.zeros(shape=(0, self.time_horizon) + shape, dtype=tf.float32)
