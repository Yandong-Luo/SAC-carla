...
class PPOAgent(Agent):
    ...
    def __init__(self, *args, policy_lr: Union[float, LearningRateSchedule, DynamicParameter] = 1e-3, gamma=0.99,
                 ...
                 advantage_scale: Union[float, LearningRateSchedule, DynamicParameter] = 2.0, 
                 alpha_opt_lr: Union[float, LearningRateSchedule, DynamicParameter] = 1e-3,
                 q1_opt_lr: Union[float, LearningRateSchedule, DynamicParameter] = 1e-3,
                 q2_opt_lr: Union[float, LearningRateSchedule, DynamicParameter] = 1e-3, **kwargs):
        ...
        ######################################################
        # Optimization for SAC
        self.Q1_lr = DynamicParameter.create(value=q1_opt_lr)
        self.Q2_lr = DynamicParameter.create(value=q2_opt_lr)
        self.alpha_opt_lr = DynamicParameter.create(value=alpha_opt_lr)

        self.Q1_optimizer = utils.get_optimizer_by_name(optimizer, learning_rate=self.Q1_lr)
        self.Q2_optimizer = utils.get_optimizer_by_name('rmsprop', learning_rate=self.Q2_lr)
        self.alpha_optimizer = utils.get_optimizer_by_name('sgd', learning_rate=self.alpha_opt_lr)
        ######################################################
    ...
    #     print("policy batch type",type(policy_batches))
    ...
    #             # self.update_alpha(policy_grads)
    ...
    #     # alpha optimization:
    #     # for _ in range(self.optimization_steps['alpha']):
    #     #     for 
    ...
    def update(self):
        ...
        for _ in range(self.optimization_steps['policy']):
            for data_batch in batches:
                self.seed_regularization()
                self.get_all_update(data_batch)
        ...
        #     for data_batch in policy_batches:
        #         self.seed_regularization()
        #         total_loss, alpha_loss, policy_grads, alpha_grads = self.get_policy_gradients(data_batch)
        #         print("ready to update")
        #         self.update_policy(policy_grads)
        #         self.alpha_optimizer.apply_gradients(zip(alpha_grads, [self.log_aplha]))
    ...
    #         states, advantages, old_log_prob, true_speed, true_similarity  = data_batch
    ...
    #             new_Q_value = tf.math.minimum(self.network.Q_value_network1([states, policy_action])['q_value'], self.network.Q_value_network2([states, policy_action])['q_value'])
    ...
    #             total_policy_loss = policy_loss + speed_loss + similarity_loss
    ...
    #         policy_gradients = tape.gradient(policy_loss, self.network.policy.trainable_variables)
    #         dynamics_gradients = tape.gradient(policy_loss, self.network.dynamics.trainable_variables)
    #         policy_grad = dict(policy=policy_gradients, dynamics=dynamics_gradients)
    #         self.update_policy(policy_grad)
    ...
    def apply_policy_gradients(self, gradients):
        print("apply_policy_gradients apply_policy_gradients apply_policy_gradients")
    ...
    def all_batch_tensors(self) -> Union[tuple, dict]:
        return self.memory.states, self.memory.advantages, self.memory.actions, self.memory.log_probabilities, self.memory.returns, self.memory.done, self.memory.next_states
    ...
    def get_all_batches(self):
        return utils.data_to_batches(tensors=self.all_batch_tensors(), batch_size=self.batch_size,
                                     drop_remainder=self.drop_batch_remainder, skip=self.skip_count,
                                     num_shards=self.obs_skipping, shuffle=self.shuffle,
                                     shuffle_batches=self.shuffle_batches)
    ...
    def policy_objective(self, batch):
        """PPO-Clip Objective"""
        print("PPO-Clip Objective PPO-Clip Objective")
    ...
    def learn(self, episodes: int, timesteps: int, save_every: Union[bool, str, int] = False,
        ...
        # print("learning000000000000000000000000000000000000000000")
        ...
        try:
            ...
            for episode in range(1, episodes + 1):
                ...
                for t in range(1, timesteps + 1):
                    ...
                    # print("state",state)
                    next_state_dic = copy.deepcopy(next_state)
                    if isinstance(next_state_dic, dict):
                        next_state_dic = {f'state_{k}': v for k, v in next_state_dic.items()}
                    next_state_dic = preprocess_fn(next_state_dic)
                    next_state_dic = utils.to_tensor(next_state_dic)
                    # print("next state", next_state_dic)
                    self.memory.append(state, action, reward, value, log_prob, done, next_state_dic)
                    state = next_state
...
class PPOMemory:
    ...
    def __init__(self, state_spec: dict, num_actions: int):
        ...
        if list(state_spec.keys()) == ['state']:
            ...
            self.next_states = tf.zeros(shape=(0,) + state_spec.get('state'), dtype=tf.float32)
            ...
            for name, shape in state_spec.items():
                self.next_states[name] = tf.zeros(shape=(0,) + shape, dtype=tf.float32)
    ...
    def append(self, state, action, reward, value, log_prob, done, next_state):
        ...
        if self.simple_state:
            ...
            for k, v in next_state.items():
                self.next_states[k] = tf.concat([self.next_states[k], v], axis=0)
    ...
    def update_index(self, append=False):
        print("update_indexupdate_indexupdate_indexupdate_indexupdate_indexupdate_index")
        if append:
    ...
    def serialize(self, episode: int, save_path: str):
        """Writes to file (npz - numpy compressed format) all the transitions (state, reward, action) collected so
           far.
        """
        print("serializeserializeserializeserializeserializeserialize")
        # Trace's file path:
